...
{
    public class NothiAllNoteServices : INothiAllNoteServices
    {
        IRepository<NoteItem> _noteItem;
        IRepository<NoteSaveItemAction> _noteSaveItemAction;
        public NothiAllNoteServices( IRepository<NoteItem> noteItem, IRepository<NoteSaveItemAction> noteSaveItemAction)
        {
            _noteItem = noteItem;
            _noteSaveItemAction = noteSaveItemAction;
        }
        public NothiListInboxNoteResponse GetNothiAllNote(DakUserParam dakListUserParam, string eachNothiId, string note_category, string note_order)
        {
            ...
            try
            {
                var client = new RestClient(GetAPIDomain() + GetNothiAllNoteEndPoint());
                client.Timeout = -1;
        ...
        protected string GetNothiAllNoteEndPoint()
        {
            return DefaultAPIConfiguration.NothiAllNoteEndPoint;
        }
