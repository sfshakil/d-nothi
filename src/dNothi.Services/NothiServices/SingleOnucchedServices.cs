...
namespace dNothi.Services.NothiServices
{
    public class SingleOnucchedServices : ISingleOnucchedServices
    {
        ...
        public SingleOnucchedResponse GetSingleOnucched(DakUserParam dakUserParam, NothiListRecordsDTO nothiListRecordsDTO, long note_id, long onucched_id)
        {
            SingleOnucchedResponse singleOnucchedResponse = new SingleOnucchedResponse();
            
            if (!dNothi.Utility.InternetConnection.Check())
            {
                var nothiList = _noteOnuchhedListItem.Table.FirstOrDefault(a => a.nothi_id == nothiListRecordsDTO.id && a.note_id == note_id && a.onuchhed_id == onucched_id && a.office_unit_id == dakUserParam.office_unit_id && a.office_id == dakUserParam.office_id && a.designation_id == dakUserParam.designation_id);
            ...
            try
            {
                ...
                request.AddParameter("note", "{\"nothi_id\":\"" + nothiListRecordsDTO.id + "\",\"note_id\":\"" + note_id + "\", \"onucched_id\":\"" + onucched_id + "\", \"nothi_office\":\"" + nothiListRecordsDTO.office_id + "\"}");
                IRestResponse response = client.Execute(request);
                Console.WriteLine(response.Content);

                var responseJson = response.Content;
                SaveOrUpdateNothiRecords(dakUserParam, nothiListRecordsDTO.id, note_id, onucched_id, responseJson);
